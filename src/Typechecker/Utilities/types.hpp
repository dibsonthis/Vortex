...
node_ptr Typechecker::get_type(node_ptr& node, std::vector<node_ptr> bases = {}) {
    ...
    types->_Node.List().elements.erase(std::unique(types->_Node.List().elements.begin(), types->_Node.List().elements.end(), [this](node_ptr& lhs, node_ptr& rhs) { return compareNodeTypes(lhs, rhs); }), types->_Node.List().elements.end());
...
bool Typechecker::match_types(node_ptr& _type, node_ptr& _value) {
    if (!_type || !_value) {
        return false;
    }
    ...
    if (type->type == NodeType::FUNC && type->TypeInfo.is_refinement_type) {
        ...
        node_ptr res = tc_func_call(func_call, type);
    ...
    if (type->type == NodeType::FUNC) {
        ...
        if (!type->_Node.Function().return_type) {
            // Assuming () => Type
            type->_Node.Function().return_type = tc_node(type->_Node.Function().body);
        }
...
bool Typechecker::match_values(node_ptr& nodeA, node_ptr& nodeB) {
    if (!nodeA || !nodeB) {
        return false;
    }
...
bool Typechecker::compareNodeTypes(node_ptr& lhs, node_ptr& rhs) {
